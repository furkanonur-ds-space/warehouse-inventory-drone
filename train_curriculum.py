...
Once KOLAY gorevle basliyor (hedefe yakin), basardikca ZORLASTIRIYORUZ.
Her asamada onceki asamada ogrenilen model devam ettiriliyor (transfer).
"""
import os
...
# Asama asama baslangic noktalari: KOLAYDAN ZORA
STAGES = [
...
for i, stage in enumerate(STAGES):
    print(f"\n{'='*60}")
    print(f"  {stage['name']} basliyor - baslangic: {stage['start']}")
    print(f"{'='*60}\n")
    ...
    print(f"\n{stage['name']} tamamlandi! Kaydedildi: {stage_model_path}.zip")

# Son modeli ana model olarak kaydet
