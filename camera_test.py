...
def on_image(msg):
    global latest_frame
    try:
        # 1. Gazebo'dan goruntuyu al
        img = np.frombuffer(msg.data, dtype=np.uint8)
        img = img.reshape((msg.height, msg.width, 3))
        frame = cv2.cvtColor(img, cv2.COLOR_RGB2BGR)

        # 2. Goruntuyu gri tona cevir (Islemeyi kolaylastirir)
        gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)

        # 3. KARE (KUTU) TESPIT ALGORITMASI
        # Kenarlari bul
        blurred = cv2.GaussianBlur(gray, (5, 5), 0)
        edges = cv2.Canny(blurred, 50, 150)
        
        # Cizgileri/Konturlari cikar
        contours, _ = cv2.findContours(edges, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        
        for cnt in contours:
            area = cv2.contourArea(cnt)
            # Eger sekil yeterince buyukse (Ufak tefek cizikleri yoksay)
            if area > 10000:
                ...
                if len(approx) == 4:
                    # Etrafina KALIN YESIL BIR KUTU ciz!
